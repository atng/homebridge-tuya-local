...
class SimpleFanAccessory extends BaseAccessory {
  ...
  _registerCharacteristics(dps) {
    ...
    this.dpRotationSpeed =
      this._getCustomDP(this.device.context.dpRotationSpeed) || "3";
  ...
  getSpeed(callback) {
    this.getState(this.dpRotationSpeed, (err, dp) => {
      if (err) return callback(err);
      console.log(this._getSpeed(dp), dp);
      this.log(this._getSpeed(dp), dp);
      this.log.debug(this._getSpeed(dp), dp);
