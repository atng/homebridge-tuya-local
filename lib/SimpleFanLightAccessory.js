...
class SimpleFanLightAccessory extends BaseAccessory {
  ...
  _registerCharacteristics(dps) {
    ...
    this.dpColorTemperature =
      this._getCustomDP(this.device.context.dpColorTemperature) || "3";
    ...
    if (this.useLight) {
      ...
      if (this.dpColorTemperature) {
        characteristicColorTemperature = serviceLightbulb
  ...
  setBrightness(value, callback) {
    const { Characteristic } = this.hap;
    const brightness = this.convertBrightnessFromHomeKitToTuya(value);
  ...
  getColorTemperature(callback) {
    // callback(null, this._getColorTemperature(dp));
    this.getState(this.dpColorTemperature, (err, dp) => {
      if (err) return callback(err);
  ...
  _getColorTemperature(dp) {
    const { Characteristic } = this.hap;
    return this.convertColorTemperatureFromTuyaToHomeKit(dp);
  }
